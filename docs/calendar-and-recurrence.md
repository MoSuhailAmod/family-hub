...
Event colour is the first participant's colour, or a neutral fallback when the event has no participants. Categories no longer carry a colour of their own; they remain available as semantic metadata (name, icon) for labelling and filtering. At the 760px mobile breakpoint, `globals.css` hides the sidebar, provides fixed bottom navigation, makes toolbar/view controls span their available width, and condenses FullCalendar event content by hiding time/owner/repeat details. These rules preserve interaction contracts without making this document a pixel specification.
